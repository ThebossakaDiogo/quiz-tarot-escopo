(function () {
  ...
  function renderPixResult(pix) {
    ...
    box.innerHTML =
      ...
      '<textarea id="tdl-pix-code" readonly onclick="this.select()" title="Toque para selecionar" class="mt-2.5 h-16 w-full rounded-xl border border-emerald-200 bg-white p-2.5 text-[10px] text-stone-600 outline-none focus:border-emerald-500 font-mono select-all">' + escapeHtml(pix.pix_payload) + '</textarea>' +
      ...
      '<div class="mt-4 rounded-2xl border border-stone-200 bg-stone-50/90 p-3.5 text-left text-[11.5px] text-stone-700 space-y-2">' +
        ...
        '<div class="flex items-start gap-2.5">' +
          ...
          '<span>Cole o código, confirme o pagamento e volte aqui.</span>' +
        '</div>' +
      '</div>' +
      ...
      '<div class="mt-3.5 rounded-2xl border-2 border-amber-300 bg-amber-50/90 p-3.5 text-left">' +
        ...
        '<p class="mt-1 text-[11px] leading-relaxed text-amber-900">' +
          'Assim que o banco confirmar o pagamento, nossa tela reconhece em poucos segundos e te leva direto para a consulta com a taróloga.' +
        '</p>' +
      '</div>' +

      '<div id="tdl-payment-status" class="mt-3.5 flex items-center justify-center gap-2.5 rounded-2xl border border-emerald-300 bg-emerald-100/70 p-3 text-[12px] font-bold text-emerald-950">' +
    ...
    pixTimerInterval = setInterval(function () {
      ...
      if (expiresSeconds <= 0) {
        ...
        var cd = document.getElementById("tdl-pix-countdown");
    ...
    var copyBtn = document.getElementById("tdl-copy-pix");
    if (copyBtn) {
  ...
  function watchPayment(pix) {
    ...
    var simBtnId = "tdl-sim-carta-btn";
