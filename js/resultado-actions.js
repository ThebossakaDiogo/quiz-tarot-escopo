(function () {
  ...
  function renderPixResult(pix) {
    ...
    var formBox = document.getElementById("tdl-result-modal-form");
    if (formBox) formBox.classList.add("hidden");
    var submitBtn = document.getElementById("tdl-result-confirm-pix");
    ...
    box.innerHTML =
      '<div class="flex items-center justify-between border-b border-emerald-100 pb-3">' +
        ...
        '<div class="flex items-center gap-1 rounded-full bg-amber-100 border border-amber-300 px-2.5 py-1 text-[11px] font-extrabold text-amber-900">' +
          '<span>⏳</span> <span id="tdl-pix-countdown">15:00</span>' +
        '</div>' +
      '</div>' +
      ...
      '<button type="button" id="tdl-result-copy-pix" class="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-500 via-emerald-600 to-teal-600 px-5 py-4 text-[13.5px] font-black uppercase tracking-wider text-white shadow-xl shadow-emerald-600/30 transition-all duration-200 hover:brightness-105 active:scale-95 cursor-pointer">' +
        ...
        '<span id="tdl-copy-label">Copiar Código PIX Copia e Cola</span>' +
      '</button>' +

      '<textarea id="tdl-result-pix-code" readonly onclick="this.select()" title="Toque para selecionar" class="mt-2.5 h-16 w-full rounded-xl border border-emerald-200 bg-white p-2.5 text-[10px] text-stone-600 outline-none focus:border-emerald-500 font-mono select-all">' + escapeHtml(pix.pix_payload) + '</textarea>' +
      ...
      '<div class="mt-4 rounded-2xl border border-stone-200 bg-stone-50/90 p-3.5 text-left text-[11.5px] text-stone-700 space-y-2">' +
        ...
        '<div class="flex items-start gap-2.5">' +
          ...
          '<span>Cole o código, confirme o valor de <strong>' + formatMoney(selectedAmount) + '</strong> e volte aqui.</span>' +
        '</div>' +
      '</div>' +
      ...
      '<div class="mt-3.5 rounded-2xl border-2 border-amber-300 bg-amber-50/90 p-3.5 text-left">' +
        ...
        '<p class="mt-1 text-[11px] leading-relaxed text-amber-900">' +
          'Assim que o banco confirmar o pagamento, nossa tela reconhece em menos de 10 segundos e te leva direto para a conversa com a taróloga.' +
        '</p>' +
      '</div>' +

      '<div id="tdl-result-payment-status" class="mt-3.5 flex items-center justify-center gap-2.5 rounded-2xl border border-emerald-300 bg-emerald-100/70 p-3 text-[12px] font-bold text-emerald-950">' +
        '<span class="relative flex h-3 w-3">' +
          '<span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-500 opacity-75"></span>' +
          '<span class="relative inline-flex rounded-full h-3 w-3 bg-emerald-600"></span>' +
        '</span>' +
        '<span id="tdl-status-text">Aguardando confirmação do banco...</span>' +
      '</div>';
    ...
    pixTimerInterval = setInterval(function () {
      ...
      if (expiresSeconds <= 0) {
        ...
        var timerEl = document.getElementById("tdl-pix-countdown");
    ...
    var copyBtn = document.getElementById("tdl-result-copy-pix");
    if (copyBtn) {
      copyBtn.addEventListener("click", function () {
        var codeInput = document.getElementById("tdl-result-pix-code");
        var textToCopy = (codeInput && codeInput.value) || pix.pix_payload || "";
        ...
        var label = document.getElementById("tdl-copy-label");
        if (label) label.textContent = "✓ CÓDIGO PIX COPIADO COM SUCESSO!";
        copyBtn.className = copyBtn.className.replace("from-emerald-500 via-emerald-600 to-teal-600", "from-teal-600 via-emerald-700 to-green-700") + " ring-4 ring-emerald-300";
        setTimeout(function () {
          if (label) label.textContent = "Copiar Código PIX Copia e Cola";
          copyBtn.className = copyBtn.className.replace(" ring-4 ring-emerald-300", "");
        }, 4000);
      });
    }
  ...
  function watchPayment(pix) {
    ...
    var simBtnId = "tdl-sim-confirm-btn";
    if (!document.getElementById(simBtnId)) {
      ...
      simBtn.onclick = function () {
        if (pixTimerInterval) clearInterval(pixTimerInterval);
        box.className = "mt-3.5 flex items-center justify-center gap-2.5 rounded-2xl border-2 border-emerald-500 bg-emerald-500 p-4 text-[13px] font-black text-white shadow-xl shadow-emerald-600/40 animate-pulse";
        box.innerHTML = "<span>🎉</span><span>PAGAMENTO CONFIRMADO! Conectando ao atendimento...</span>";
        setTimeout(function () {
  ...
  function createResultPix(event) {
    ...
    target.textContent = "Gerando PIX demonstrativo...";
    var info = packageInfo(selectedAmount);
    try {
  ...
  document.addEventListener("click", function (event) {
    ...
    if (
      ...
      event.preventDefault();
      go("sessao-individual-tarologa-milena-z61w?source=skipped&next=contato/&step=result");
      return;
    }
    ...
    if (
      ...
      event.preventDefault();
      go("portal-sagrado-mapa-amoroso-inten-c84m");
    }
  }, true);
